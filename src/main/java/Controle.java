...
public class Controle {
	...
	public boolean emprestar(String aluno, int[] prazos, int num) {
		...
		if (retorno) {
			...
			/*
			 * Para cada livro verifica se � exemplar da biblioteca e s� deixar� emprestar
			 * os livros que n�o s�o
			 */
			for (int i = 0; i < num; i++) {
			...
			/*
			 * Chama o m�todo delegado do aluno de emprestar cliente, passando o conjunto de
			 * livros como parametro caso tenha pelo menos um livro a ser emprestado
			 */
			if (livros.size() > 0 && livros.size() <= 10) {
				retorno = a.emprestar(livros, num);
				return retorno;
			} else
				return false;
